...
namespace Tests
{
	class MySQLConnectionStringProviderTests
	{
		...
		{
			var result = _connectionStringFactory.GetDatabaseConnectionString(_databaseConfig);
		...
		{
			var result = _connectionStringFactory.GetServerConnectionString(_databaseConfig);
			var expectedResult = $"Data Source = {_databaseConfig.ServerAddress}; Port ={_databaseConfig.ServerPort}; User ID = {_databaseConfig.UserName}; Password ={_databaseConfig.Password}; pooling = true; SslMode ={_databaseConfig.UseSSL}; connection lifetime = 300; Treat Tiny As Boolean = false; character set = utf8; AllowPublicKeyRetrieval = True;";
